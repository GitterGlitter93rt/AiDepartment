...
**This is the only block to paste into the message_flow field.** It replaces the text submitted before the 30923 rejection. Every sentence is verifiable on the live page by the checklist in §10 — nothing here describes an intention.

```
Customers may optionally opt in to receive customer-care SMS messages from Your AI Department, a business brand operated by Catastrophic Solutions LLC. SMS consent is collected on one public web page, https://youraidepartment.ai/sms-consent/, which requires no login, no account and no purchase to reach.

On that page the SMS consent checkbox is separate from every other permission, is unchecked by default, carries no required attribute, is never pre-selected by script, and is not required to submit the form, purchase services, create an account, or otherwise use our services. A second, clearly separate checkbox collects agreement to our Terms of Use and Privacy Policy. That agreement contains no permission to send text messages, and our Terms state explicitly that accepting them is not an SMS opt-in. The two checkboxes are labeled Required and Optional and are independent of each other.

The form submits successfully whether or not the SMS checkbox is selected. A customer who leaves it unselected receives a confirmation stating that they did not opt in, that no text messages will be sent, and that they can still contact us, complete an assessment, schedule a call, purchase, and receive every service we offer. Customers who wish to receive SMS messages affirmatively select the optional SMS checkbox and are shown a separate confirmation that consent was recorded. We store the phone number, the Terms agreement, and the SMS answer as three separate fields, so a phone number on file is never treated as consent.

The disclosure shown immediately beside the SMS checkbox, before consent is given, reads: "I agree to receive customer-care text messages from Your AI Department, operated by Catastrophic Solutions LLC, at the mobile number provided. Messages may include requested follow-up, appointment coordination or reminders, and support. Message frequency varies. Msg & data rates may apply. Reply STOP to opt out or HELP for help. Consent is not a condition of purchase. See our Privacy Policy and Terms of Use." The Privacy Policy and Terms of Use are clickable links on that disclosure.

Message frequency varies. Message and data rates may apply. Customers may reply STOP to opt out or HELP for assistance. Our Privacy Policy, Terms of Use, and SMS consent information are publicly accessible: https://youraidepartment.ai/privacy/ https://youraidepartment.ai/terms/ https://youraidepartment.ai/sms-consent/

We also record verbal consent during live conversations with people who have already contacted us, after reading the same disclosure aloud and receiving a clear affirmative answer. We record the mobile number, date and time, representative, consent source, script version, and the affirmative response.

No other website form collects SMS consent. Providing a phone number on our contact form or in an assessment does not enroll anyone in SMS messaging, those forms state this beside the phone field, and no form anywhere on the website requires an SMS permission in order to submit. We never send an SMS to ask someone to opt in.
```

Two accuracy corrections against the language originally drafted for this field, both of which would have contradicted the live site if pasted as written:

- **"through our website forms" → one page.** Only `/sms-consent/` collects SMS consent. Describing consent as coming from "our website forms" invites a reviewer to look for a checkbox on the contact form and assessments, find none, and read the submission as inaccurate.
- **"Terms of Service" → "Terms of Use".** The site, the footer and the disclosure all say Terms of Use. A different name for the same document is exactly the kind of mismatch that produced the original 30907 rejection.
...
The name, phone number and consent text go to the lead delivery destination only. **None of it reaches GA4, GTM or Meta.** A single non-PII diagnostic event (`sms_consent_submit`, carrying only `consent_version`, `source_page` and `sms_opt_in: yes|no`) fires after delivery succeeds. It is not a lead and not a booking.

> **One GTM action follows this deployment.** The `sms_opt_in` parameter on `sms_consent_submit` is new, and it exists so a decline and a consent are never one indistinguishable number. In container `GTM-5G8Q7KKZ`, add a Data Layer Variable `DLV - sms_opt_in` (Version 2, no default) and add `sms_opt_in` → `{{DLV - sms_opt_in}}` to the `GA4 - SMS Consent Submit` tag. Edit the existing tag rather than creating a second one, and leave the event as **not** a Key Event. A tag that forwards this event without the parameter is worse than no tag, because it reports declines as opt-ins. Nothing about the Twilio submission depends on this; it is analytics hygiene, and it can be done after resubmitting.
...
- The GTM follow-up for the new `sms_opt_in` parameter is written out in §9 above rather than linked, because the analytics operator documents live on the separate Sprint 15 branch and are not part of this production deployment.
