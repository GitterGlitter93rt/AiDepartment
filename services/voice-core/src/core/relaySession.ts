...
import { createTimeline, type Timeline, type TimelineSink } from './telemetry.ts';
import { parseRelayMessage, textResponse } from '../twilio/relay.ts';
import { chunkForSpeech } from '../twilio/relay.ts';

/**
 * A ConversationRelay session.
 *
 * Transport only: it carries words and decides nothing about them. The turn producer
 * is injected, so the whole session can be exercised without Twilio, without a
 * socket, and without a database -- which is what the tests do.
 *
 * It lives in voice-core because outbound sales and inbound callback handling need
 * exactly the same turn machinery and must not drift apart. What differs between the
 * two services is the producer, the persona and the routing; none of that is here.
 *
...
export function createRelaySession(input: {
  ...
  const sessions = new SessionStore<RelaySessionState>(
