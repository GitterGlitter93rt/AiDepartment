...
//
// Ported from services/ai-phone-agent at 2ad6449. One mark did not come across: the
// one that timed the demo line's split branded intro, which is demo behaviour rather
// than transport.
//
  ...
  return {
    ...
    since(mark) {
      ...
      for (let i = marks.length - 1; i >= 0; i -= 1) {
        ...
        if (record.mark === mark) return (now() - t0) - record.atMs;
      }
      return undefined;
    },
    beginTurn() {
