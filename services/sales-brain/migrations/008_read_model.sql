-- 008_read_model.sql — search projection.
-- Canonical authority: rep-ownership-data-model.md §16 ("a cache/projection, not
-- canonical truth"), rep-inventory-contract.v1.yaml result_summary.
--
-- Writes always go through the canonical tables. This view exists so the prospect
-- table does not join every evidence row on every keystroke.
...
comment on view prospect_inventory is
  'Search projection over canonical entities. Read-only: all writes go through the canonical tables.';
